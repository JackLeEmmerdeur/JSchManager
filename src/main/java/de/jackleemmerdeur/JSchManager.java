...
public class JSchManager
    ...
    protected static final String ERR_SHELL_OPENED = "Error after opening shell: %s";
    ...
    public void close()
        ...
        if (channels != null) {
            ...
            for (Map.Entry<String, ArrayList<SSHChannel>> entry : channels.entrySet()) {
                sessionchannels = entry.getValue();
        ...
        if (sessions != null) {
            ...
            for (Map.Entry<String, SSHSession> entry : sessions.entrySet()) {
                SSHSession s = entry.getValue();
    ...
    public SSHChannelSftp openChannelSFTP(SSHSession session, boolean connectImmediately, Integer connecttimeoutMS)
        ...
        if (connectImmediately && sftpchannel != null)
            sftpchannel.connectChannel(SSHChannel.getTimeout(connecttimeoutMS));
        return sftpchannel;
    }
    ...
    public SSHChannelShell openChannelShell(String sessionID, int connectionTimeout)
            throws Exception {
        return openChannelShell(sessionID, true, true, connectionTimeout, -1, -1, -1, -1);
    }

    public SSHChannelShell openChannelShell(SSHSession session, int connectionTimeout)
            throws Exception {
        return openChannelShell(session, true, true, connectionTimeout, -1, -1, -1, -1);
    }
